...
class grid_nurses_pesq
{
   ...
   function monta_busca()
   {
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_nurses']['path_libs_php'] = $this->Ini->path_lib_php;
...
//-- ajax metodos ---
      if ($this->NM_ajax_opcao == "ajax_filter_save")
      {
          ...
          foreach ($this->NM_fil_ant as $Cada_filter => $Tipo_filter)
          {
              ...
              if ($Tipo_filter[1] != $Nome_filter)
              {
                  ...
                  $Opt_filter .= "<option value=\"\">" . grid_nurses_pack_protect_string($Nome_filter) . "</option>\r\n";
              }
              $Opt_filter .= "<option value=\"" . grid_nurses_pack_protect_string($Tipo_filter[0]) . "\">.." . grid_nurses_pack_protect_string($Cada_filter) .  "</option>\r\n";
          }
      ...
      if ($this->NM_ajax_opcao == 'autocomp_idnurse')
      {
          $idnurse = ($_SESSION['scriptcase']['charset'] != "UTF-8" && NM_is_utf8($_GET['q'])) ? sc_convert_encoding($_GET['q'], $_SESSION['scriptcase']['charset'], "UTF-8") : $_GET['q'];
          $nmgp_def_dados = $this->lookup_ajax_idnurse($idnurse);
          ob_end_clean();
          ob_end_clean();
          $count_aut_comp = 0;
          $resp_aut_comp  = array();
      ...
      if ($this->NM_ajax_opcao == 'autocomp_namenurse')
      {
          $namenurse = ($_SESSION['scriptcase']['charset'] != "UTF-8" && NM_is_utf8($_GET['q'])) ? sc_convert_encoding($_GET['q'], $_SESSION['scriptcase']['charset'], "UTF-8") : $_GET['q'];
          $nmgp_def_dados = $this->lookup_ajax_namenurse($namenurse);
          ob_end_clean();
          ob_end_clean();
          $count_aut_comp = 0;
          $resp_aut_comp  = array();
   ...
   function lookup_ajax_idnurse($idnurse)
   {
      $idnurse = str_replace($_SESSION['scriptcase']['reg_conf']['grup_num'], "", $idnurse);
      $idnurse = substr($this->Db->qstr($idnurse), 1, -1);
            $idnurse_look = (is_string($idnurse) ? substr($this->Db->qstr($idnurse), 1, -1) : $idnurse); 
      $nmgp_def_dados = array(); 
      if (isset($this->Ini->nm_bases_postgres) && in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_postgres))
      {
          $nm_comando = "select distinct idnurse from " . $this->Ini->nm_tabela . " where   CAST (idnurse AS TEXT)  like '%" . $idnurse . "%' order by idnurse"; 
      }
      elseif (isset($this->Ini->nm_bases_sybase) && in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_sybase))
      {
          $nm_comando = "select distinct idnurse from " . $this->Ini->nm_tabela . " where   CAST (idnurse AS VARCHAR)  like '%" . $idnurse . "%' order by idnurse"; 
      }
      else
      {
          $nm_comando = "select distinct idnurse from " . $this->Ini->nm_tabela . " where  idnurse like '%" . $idnurse . "%' order by idnurse"; 
      }
      unset($cmp1,$cmp2);
   ...
   function lookup_ajax_namenurse($namenurse)
   {
      $namenurse = substr($this->Db->qstr($namenurse), 1, -1);
            $namenurse_look = (is_string($namenurse) ? substr($this->Db->qstr($namenurse), 1, -1) : $namenurse); 
      $nmgp_def_dados = array(); 
      $nm_comando = "select distinct namenurse from " . $this->Ini->nm_tabela . " where  namenurse like '%" . $namenurse . "%' order by namenurse"; 
      unset($cmp1,$cmp2);
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando; 
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = ''; 
      if ($rs = $this->Db->Execute($nm_comando)) 
   ...
   {
      $login = str_replace($_SESSION['scriptcase']['reg_conf']['grup_num'], "", $login);
      $login = substr($this->Db->qstr($login), 1, -1);
            $login_look = (is_string($login) ? substr($this->Db->qstr($login), 1, -1) : $login); 
      $nmgp_def_dados = array(); 
      if (isset($this->Ini->nm_bases_postgres) && in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_postgres))
      {
          $nm_comando = "select distinct login from " . $this->Ini->nm_tabela . " where   CAST (login AS TEXT)  like '%" . $login . "%' order by login"; 
      }
      elseif (isset($this->Ini->nm_bases_sybase) && in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_sybase))
      {
          $nm_comando = "select distinct login from " . $this->Ini->nm_tabela . " where   CAST (login AS VARCHAR)  like '%" . $login . "%' order by login"; 
      }
      else
      {
      ...
      else
      {
         ...
         switch ($condicao)
         {
            case "EQ":     // 
               ...
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_nurses']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_equl'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
            case "II":     // 
               ...
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_nurses']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_strt'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
             case "QP";     // 
             case "NP";     // 
               ...
               if ($this->Date_part)
               {
                   if (!empty($NM_cmd))
                   {
                       ...
                       $_SESSION['sc_session'][$this->Ini->sc_page]['grid_nurses']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . ": " . $NM_cond . "##*@@";
                   }
               }
               else
               {
                   ...
                   $this->comando_sum    .= $nm_ini_lower . $Nm_accent['cmp_i'] . $nome_sum . $Nm_accent['cmp_f'] . $nm_fim_lower . $Nm_accent['cmp_apos'] . $op_all . $nm_ini_lower . "'%" . $Nm_accent['arg_i'] . $campo . $Nm_accent['arg_f'] . "%'" . $nm_fim_lower . $Nm_accent['arg_apos'];
                   $this->comando_filtro .= $nm_ini_lower . $Nm_accent['cmp_i'] . $nome . $Nm_accent['cmp_f'] . $nm_fim_lower . $Nm_accent['cmp_apos'] . $op_all . $nm_ini_lower . "'%" . $Nm_accent['arg_i'] . $campo . $Nm_accent['arg_f'] . "%'" . $nm_fim_lower . $Nm_accent['arg_apos'];
                   $_SESSION['sc_session'][$this->Ini->sc_page]['grid_nurses']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $lang_like . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
               }
            break;
            case "DF":     // 
               $this->comando        .= $nm_ini_lower . $nome . $nm_fim_lower . " <> " . $nm_ini_lower . $nm_aspas . $campo . $nm_aspas1 . $nm_fim_lower;
               $this->comando_sum    .= $nm_ini_lower . $nome_sum . $nm_fim_lower . " <> " . $nm_ini_lower . $nm_aspas . $campo . $nm_aspas1 . $nm_fim_lower;
               $this->comando_filtro .= $nm_ini_lower . $nome . $nm_fim_lower . " <> " . $nm_ini_lower . $nm_aspas . $campo . $nm_aspas1 . $nm_fim_lower;
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_nurses']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_diff'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
            case "GT":     // 
               ...
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_nurses']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_grtr'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
            case "GE":     // 
               $this->comando        .= " $nome >= " . $nm_aspas . $campo . $nm_aspas1;
               $this->comando_sum    .= " $nome_sum >= " . $nm_aspas . $campo . $nm_aspas1;
               $this->comando_filtro .= " $nome >= " . $nm_aspas . $campo . $nm_aspas1;
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_nurses']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_grtr_equl'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
            case "LT":     // 
               $this->comando        .= " $nome < " . $nm_aspas . $campo . $nm_aspas1;
               $this->comando_sum    .= " $nome_sum < " . $nm_aspas . $campo . $nm_aspas1;
               $this->comando_filtro .= " $nome < " . $nm_aspas . $campo . $nm_aspas1;
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_nurses']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_less'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
            case "LE":     // 
               $this->comando        .= " $nome <= " . $nm_aspas . $campo . $nm_aspas1;
               $this->comando_sum    .= " $nome_sum <= " . $nm_aspas . $campo . $nm_aspas1;
               $this->comando_filtro .= " $nome <= " . $nm_aspas . $campo . $nm_aspas1;
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_nurses']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_less_equl'] . " " . $this->cmp_formatado[$nome_campo] . "##*@@";
            break;
            case "BW":     // 
               ...
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_nurses']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_betw'] . " " . $this->cmp_formatado[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_and_cond'] . " " . $this->cmp_formatado[$nome_campo . "_input_2"] . "##*@@";
            break;
            case "IN":     // 
               ...
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_nurses']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_like'] . " " . $nm_cond . "##*@@";
            break;
            case "NU":     // 
               $this->comando        .= " $nome IS NULL ";
               $this->comando_sum    .= " $nome_sum IS NULL ";
               $this->comando_filtro .= " $nome IS NULL ";
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_nurses']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_null'] . "##*@@";
            break;
            case "NN":     // 
               $this->comando        .= " $nome IS NOT NULL ";
               $this->comando_sum    .= " $nome_sum IS NOT NULL ";
               $this->comando_filtro .= " $nome IS NOT NULL ";
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_nurses']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_nnul'] . "##*@@";
            break;
            case "EP":     // 
               $this->comando        .= " $nome = '' ";
               $this->comando_sum    .= " $nome_sum = '' ";
               $this->comando_filtro .= " $nome = '' ";
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_nurses']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_empty'] . "##*@@";
            break;
            case "NE":     // 
               $this->comando        .= " $nome <> '' ";
               $this->comando_sum    .= " $nome_sum <> '' ";
               $this->comando_filtro .= " $nome <> '' ";
               $_SESSION['sc_session'][$this->Ini->sc_page]['grid_nurses']['cond_pesq'] .= $nmgp_tab_label[$nome_campo] . " " . $this->Ini->Nm_lang['lang_srch_nempty'] . "##*@@";
            break;
         }
      }
   }
...
<HTML>
<HEAD>
 <TITLE><?php echo $this->Ini->Nm_lang['lang_tbl_nurses'] ?></TITLE>
...
?>
 ...
 <link rel="stylesheet" type="text/css" href="../_lib/css/<?php echo $this->Ini->str_schema_filter ?>_filter<?php echo $_SESSION['scriptcase']['reg_conf']['css_dir'] ?>.css" /> 
  ...
  ?> 
 <link rel="stylesheet" type="text/css" href="<?php echo $this->Ini->path_link ?>grid_nurses/grid_nurses_fil_<?php echo strtolower($_SESSION['scriptcase']['reg_conf']['css_dir']) ?>.css" />
</HEAD>
<?php
$vertical_center = '';
?>
...
<?php echo $this->Ini->Ajax_result_set ?>
 ...
 <script type="text/javascript" src="grid_nurses_ajax_search.js"></script>
 <script type="text/javascript" src="grid_nurses_ajax.js"></script>
 <script type="text/javascript">
...
<script type="text/javascript" src="grid_nurses_message.js"></script>
<link rel="stylesheet" type="text/css" href="../_lib/css/<?php echo $this->Ini->str_schema_filter ?>_sweetalert.css" />
<script type="text/javascript" src="<?php echo $_SESSION['scriptcase']['grid_nurses']['glo_nm_path_prod']; ?>/third/sweetalert/sweetalert2.all.min.js"></script>
<script type="text/javascript" src="<?php echo $_SESSION['scriptcase']['grid_nurses']['glo_nm_path_prod']; ?>/third/sweetalert/polyfill.min.js"></script>
...
var nmdg_Form = "F1";
 ...
 function nm_save_form(pos)
 {
  ...
  var str_out = "";
  str_out += 'SC_idnurse_cond#NMF#' + search_get_sel_txt('SC_idnurse_cond') + '@NMF@';
  str_out += 'SC_idnurse#NMF#' + search_get_text('SC_idnurse') + '@NMF@';
  str_out += 'id_ac_idnurse#NMF#' + search_get_title('select2-id_ac_idnurse-container') + '@NMF@';
  str_out += 'SC_namenurse_cond#NMF#' + search_get_sel_txt('SC_namenurse_cond') + '@NMF@';
  str_out += 'SC_namenurse#NMF#' + search_get_text('SC_namenurse') + '@NMF@';
  str_out += 'id_ac_namenurse#NMF#' + search_get_title('select2-id_ac_namenurse-container') + '@NMF@';
  str_out += 'SC_login_cond#NMF#' + search_get_sel_txt('SC_login_cond') + '@NMF@';
  str_out += 'SC_login#NMF#' + search_get_text('SC_login') + '@NMF@';
  str_out += 'id_ac_login#NMF#' + search_get_title('select2-id_ac_login-container') + '@NMF@';
...
<script type="text/javascript">
 $(function() {
  $(".sc-ui-autocomp-idnurse").on("focus", function() {
  }).on("blur", function() {
   ...
   ajax: {
    ...
    data: function (params) {
     var query = {
      ...
      nmgp_parms: "NM_ajax_opcao?#?autocomp_idnurse",
      max_itens: "10",
   ...
   $("#SC_idnurse").val(e.params.data.id);
  });
  $(".sc-ui-autocomp-namenurse").on("focus", function() {
  }).on("blur", function() {
  }).on("keydown", function(e) {
   if(e.keyCode == $.ui.keyCode.TAB && $(".ui-autocomplete").filter(":visible").length) {
    e.keyCode = $.ui.keyCode.DOWN;
   ...
   ajax: {
    ...
    data: function (params) {
     var query = {
      q: params.term,
      nmgp_opcao: "ajax_autocomp",
      nmgp_parms: "NM_ajax_opcao?#?autocomp_namenurse",
      max_itens: "10",
      script_case_init: <?php echo $this->Ini->sc_page ?>
     }
     return query;
   ...
   $("#SC_namenurse").val(e.params.data.id);
  });
  $(".sc-ui-autocomp-login").on("focus", function() {
  }).on("blur", function() {
  }).on("keydown", function(e) {
   if(e.keyCode == $.ui.keyCode.TAB && $(".ui-autocomplete").filter(":visible").length) {
    e.keyCode = $.ui.keyCode.DOWN;
...
<?php
   ...
   {
      if ($this->Ini->Embutida_iframe || $_SESSION['sc_session'][$this->Ini->sc_page]['grid_nurses']['dashboard_info']['compact_mode'])
      {
          return;
      }
...
<div class="scFilterHeader" style="height: 54px; padding: 17px 15px; box-sizing: border-box;margin: -1px 0px 0px 0px;width: 100%;">
    <div class="scFilterHeaderFont" style="float: left; text-transform: uppercase;"><?php echo $this->Ini->Nm_lang['lang_tbl_nurses'] ?></div>
...
<?php
   ...
   {
      global 
             $idnurse_cond, $idnurse, $idnurse_autocomp,
             $namenurse_cond, $namenurse, $namenurse_autocomp,
      ...
      $nmgp_tab_label = "";
      $delimitador = "##@@";
      if (empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_nurses']['campos_busca']) && $bprocessa != "recarga" && $bprocessa != "save_form" && $bprocessa != "filter_save" && $bprocessa != "filter_delete")
      {
      }
      if (!empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_nurses']['campos_busca']) && $bprocessa != "recarga" && $bprocessa != "save_form" && $bprocessa != "filter_save" && $bprocessa != "filter_delete")
      { 
          if ($_SESSION['scriptcase']['charset'] != "UTF-8")
          {
              $_SESSION['sc_session'][$this->Ini->sc_page]['grid_nurses']['campos_busca'] = NM_conv_charset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_nurses']['campos_busca'], $_SESSION['scriptcase']['charset'], "UTF-8");
          }
          $idnurse = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_nurses']['campos_busca']['idnurse']; 
          $idnurse_cond = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_nurses']['campos_busca']['idnurse_cond']; 
          $namenurse = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_nurses']['campos_busca']['namenurse']; 
          $namenurse_cond = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_nurses']['campos_busca']['namenurse_cond']; 
          $login = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_nurses']['campos_busca']['login']; 
          $login_cond = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_nurses']['campos_busca']['login_cond']; 
          $this->NM_operador = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_nurses']['campos_busca']['NM_operador']; 
      } 
      if (!isset($idnurse_cond) || empty($idnurse_cond))
      {
         $idnurse_cond = "gt";
      }
      if (!isset($namenurse_cond) || empty($namenurse_cond))
      {
         $namenurse_cond = "qp";
      }
      ...
      $str_hide_idnurse = (in_array($idnurse_cond, $opc_hide_input)) ? $display_fechado : $display_aberto;
      $str_hide_namenurse = (in_array($namenurse_cond, $opc_hide_input)) ? $display_fechado : $display_aberto;
      $str_hide_login = (in_array($login_cond, $opc_hide_input)) ? $display_fechado : $display_aberto;

      if (!isset($idnurse) || $idnurse == "")
      {
          $idnurse = "";
      }
      if (isset($idnurse) && !empty($idnurse))
      {
         $tmp_pos = (is_string($idnurse)) ? strpos($idnurse, "##@@") : false;
         if ($tmp_pos === false)
         { }
         else
         {
         $idnurse = substr($idnurse, 0, $tmp_pos);
         }
      }
      if (!isset($namenurse) || $namenurse == "")
      {
          $namenurse = "";
      }
      if (isset($namenurse) && !empty($namenurse))
      {
         $tmp_pos = (is_string($namenurse)) ? strpos($namenurse, "##@@") : false;
         if ($tmp_pos === false)
         { }
         else
         {
         $namenurse = substr($namenurse, 0, $tmp_pos);
         }
      }
...
?>
<?php
   if (is_file("grid_nurses_help.txt"))
   {
...
?>
<?php
   if (isset($_SESSION['scriptcase']['sc_apl_conf']['grid_nurses']['start']) && $_SESSION['scriptcase']['sc_apl_conf']['grid_nurses']['start'] == 'filter' && $nm_apl_dependente != 1)
   {
?>
...
?>
 ...
 <TR align="center">
  ...
  <TD width="100%" height="">
   ...
   <tr valign="top">
      ...
      <TD id='SC_idnurse_label' class="scFilterLabelOdd"><?php echo (isset($this->New_label['idnurse'])) ? $this->New_label['idnurse'] : "" . $this->Ini->Nm_lang['lang_nurses_fld_idnurse'] . ""; ?></TD>
     <TD class="scFilterFieldOdd"> 
      <SELECT class="scFilterObjectOdd" id="SC_idnurse_cond" name="idnurse_cond" onChange="nm_campos_between(document.getElementById('id_vis_idnurse'), this, 'idnurse')">
       <OPTION value="gt" <?php if ("gt" == $idnurse_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_grtr'] ?></OPTION>
       <OPTION value="lt" <?php if ("lt" == $idnurse_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_less'] ?></OPTION>
       <OPTION value="eq" <?php if ("eq" == $idnurse_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_exac'] ?></OPTION>
      </SELECT>
       </TD>
     <TD  class="scFilterFieldOdd">
      <TABLE  border="0" cellpadding="0" cellspacing="0">
       <TR id="id_hide_idnurse" <?php echo $str_hide_idnurse?> valign="top">
 ...
 $nmgp_tab_label .= "idnurse?#?" . $SC_Label . "?@?";
 $date_sep_bw = " ";
...
<?php
      ...
      $idnurse_look = (is_string($idnurse) ? substr($this->Db->qstr($idnurse), 1, -1) : $idnurse); 
      $nmgp_def_dados = array(); 
   if (is_numeric($idnurse))
   { 
      if (isset($this->Ini->nm_bases_postgres) && in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_postgres))
      {
          $nm_comando = "select distinct idnurse from " . $this->Ini->nm_tabela . " where idnurse = $idnurse_look order by idnurse"; 
      }
      elseif (isset($this->Ini->nm_bases_sybase) && in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_sybase))
      {
          $nm_comando = "select distinct idnurse from " . $this->Ini->nm_tabela . " where idnurse = $idnurse_look order by idnurse"; 
      ...
      if (isset($nmgp_def_dados[0][$idnurse]))
      {
          $sAutocompValue = $nmgp_def_dados[0][$idnurse];
      }
      else
      {
            nmgp_Form_Num_Val($idnurse, $_SESSION['scriptcase']['reg_conf']['grup_num'], $_SESSION['scriptcase']['reg_conf']['dec_num'], "0", "S", "2", "", "N:" . $_SESSION['scriptcase']['reg_conf']['neg_num'] , $_SESSION['scriptcase']['reg_conf']['simb_neg'], $_SESSION['scriptcase']['reg_conf']['num_group_digit']) ; 
          $sAutocompValue = $idnurse;
      }
?>
<INPUT  type="text" id="SC_idnurse" name="idnurse" value="<?php echo NM_encode_input($idnurse) ?>" size=11 alt="{datatype: 'integer', maxLength: 11, thousandsSep: '<?php echo $_SESSION['scriptcase']['reg_conf']['grup_num'] ?>', allowNegative: false, onlyNegative: false, enterTab: false}" style="display: none">
<select class="sc-js-input scFilterObjectOdd sc-ui-autocomp-idnurse" id="id_ac_idnurse" name="idnurse_autocomp"><?php if ('' !=  $idnurse) { ?><option value="<?php echo $idnurse ?>" selected><?php echo $sAutocompValue ?></option><?php } ?></select>
      ...
      <TD id='SC_namenurse_label' class="scFilterLabelEven"><?php echo (isset($this->New_label['namenurse'])) ? $this->New_label['namenurse'] : "" . $this->Ini->Nm_lang['lang_nurses_fld_namenurse'] . ""; ?></TD>
     <TD class="scFilterFieldEven"> 
      <SELECT class="scFilterObjectEven" id="SC_namenurse_cond" name="namenurse_cond" onChange="nm_campos_between(document.getElementById('id_vis_namenurse'), this, 'namenurse')">
       <OPTION value="qp" <?php if ("qp" == $namenurse_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_like'] ?></OPTION>
       <OPTION value="np" <?php if ("np" == $namenurse_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_not_like'] ?></OPTION>
       <OPTION value="eq" <?php if ("eq" == $namenurse_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_exac'] ?></OPTION>
       <OPTION value="ep" <?php if ("ep" == $namenurse_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_empty'] ?></OPTION>
      </SELECT>
       </TD>
     <TD  class="scFilterFieldEven">
      <TABLE  border="0" cellpadding="0" cellspacing="0">
       <TR id="id_hide_namenurse" <?php echo $str_hide_namenurse?> valign="top">
 ...
 $nmgp_tab_label .= "namenurse?#?" . $SC_Label . "?@?";
 $date_sep_bw = " ";
 if ($_SESSION['scriptcase']['charset'] != "UTF-8" && NM_is_utf8($date_sep_bw))
 {
     $date_sep_bw = sc_convert_encoding($date_sep_bw, $_SESSION['scriptcase']['charset'], "UTF-8");
...
<?php
      ...
      $namenurse_look = (is_string($namenurse) ? substr($this->Db->qstr($namenurse), 1, -1) : $namenurse); 
      $nmgp_def_dados = array(); 
      $nm_comando = "select distinct namenurse from " . $this->Ini->nm_tabela . " where namenurse = '$namenurse_look' order by namenurse"; 
      unset($cmp1,$cmp2);
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando; 
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = ''; 
      if ($rs = $this->Db->Execute($nm_comando)) 
      ...
      if (isset($nmgp_def_dados[0][$namenurse]))
      {
          $sAutocompValue = $nmgp_def_dados[0][$namenurse];
      }
      else
      {
          $sAutocompValue = $namenurse;
      }
?>
<INPUT  type="text" id="SC_namenurse" name="namenurse" value="<?php echo NM_encode_input($namenurse) ?>"  size=50 alt="{datatype: 'text', maxLength: 100, allowedChars: '', lettersCase: '', autoTab: false, enterTab: false}" style="display: none">
<select class="sc-js-input scFilterObjectEven sc-ui-autocomp-namenurse" id="id_ac_namenurse" name="namenurse_autocomp"><?php if ('' !=  $namenurse) { ?><option value="<?php echo $namenurse ?>" selected><?php echo $sAutocompValue ?></option><?php } ?></select>
      ...
      <TD id='SC_login_label' class="scFilterLabelOdd"><?php echo (isset($this->New_label['login'])) ? $this->New_label['login'] : "" . $this->Ini->Nm_lang['lang_nurses_fld_login'] . ""; ?></TD>
     <TD class="scFilterFieldOdd"> 
      <SELECT class="scFilterObjectOdd" id="SC_login_cond" name="login_cond" onChange="nm_campos_between(document.getElementById('id_vis_login'), this, 'login')">
       <OPTION value="gt" <?php if ("gt" == $login_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_grtr'] ?></OPTION>
       <OPTION value="lt" <?php if ("lt" == $login_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_less'] ?></OPTION>
...
<?php
   ...
   { 
      ...
      {
            nmgp_Form_Num_Val($login, $_SESSION['scriptcase']['reg_conf']['grup_num'], $_SESSION['scriptcase']['reg_conf']['dec_num'], "0", "S", "2", "", "N:" . $_SESSION['scriptcase']['reg_conf']['neg_num'] , $_SESSION['scriptcase']['reg_conf']['simb_neg'], $_SESSION['scriptcase']['reg_conf']['num_group_digit']) ; 
          $sAutocompValue = $login;
      }
?>
<INPUT  type="text" id="SC_login" name="login" value="<?php echo NM_encode_input($login) ?>" size=11 alt="{datatype: 'integer', maxLength: 11, thousandsSep: '<?php echo $_SESSION['scriptcase']['reg_conf']['grup_num'] ?>', allowNegative: false, onlyNegative: false, enterTab: false}" style="display: none">
<select class="sc-js-input scFilterObjectOdd sc-ui-autocomp-login" id="id_ac_login" name="login_autocomp"><?php if ('' !=  $login) { ?><option value="<?php echo $login ?>" selected><?php echo $sAutocompValue ?></option><?php } ?></select>
...
?>
   <INPUT type="hidden" name="script_case_init" value="<?php echo NM_encode_input($this->Ini->sc_page); ?>"> 
<?php
   if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_nurses']['orig_pesq']) && $_SESSION['sc_session'][$this->Ini->sc_page]['grid_nurses']['orig_pesq'] == "grid")
   {
...
?>
 ...
 function limpa_form()
 {
   ...
   document.F1.idnurse_cond.value = 'gt';
   nm_campos_between(document.getElementById('id_vis_idnurse'), document.F1.idnurse_cond, 'idnurse');
   document.F1.idnurse.value = "";
   document.F1.idnurse_autocomp.value = "";
   $('#select2-id_ac_idnurse-container').html('<?php echo $this->Val_init_idnurse['desc'] ?>');;
   document.F1.namenurse_cond.value = 'qp';
   nm_campos_between(document.getElementById('id_vis_namenurse'), document.F1.namenurse_cond, 'namenurse');
   document.F1.namenurse.value = "";
   document.F1.namenurse_autocomp.value = "";
   $('#select2-id_ac_namenurse-container').html('<?php echo $this->Val_init_namenurse['desc'] ?>');;
   document.F1.login_cond.value = 'gt';
   ...
   $('#select2-id_ac_login-container').html('<?php echo $this->Val_init_login['desc'] ?>');;
   Sc_carga_select2('all');
 }
...
<?php
   ...
   function gera_array_filtros()
   {
       ...
       $NM_patch   = "ProntSaude/grid_nurses";
   ...
   function inicializa_vars()
   {
      ...
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_nurses']['where_orig']))
      {
   ...
   function init_comando()
   {
      ...
      $this->comando         = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_nurses']['where_orig'];
      $this->comando_sum     = "";
      $this->comando_filtro  = "";
   ...
   function recupera_filtro($NM_filters, $Orig_Fil)
   {
      ...
      $tp_fields['SC_idnurse_cond'] = 'cond';
      $tp_fields['SC_idnurse'] = 'text_aut';
      $tp_fields['id_ac_idnurse'] = 'select2_aut';
      $tp_fields['SC_namenurse_cond'] = 'cond';
      $tp_fields['SC_namenurse'] = 'text_aut';
      $tp_fields['id_ac_namenurse'] = 'select2_aut';
      $tp_fields['SC_login_cond'] = 'cond';
      $tp_fields['SC_login'] = 'text_aut';
      $tp_fields['id_ac_login'] = 'select2_aut';
   ...
   {
      global $idnurse_cond, $idnurse, $idnurse_autocomp,
             $namenurse_cond, $namenurse, $namenurse_autocomp,
             $login_cond, $login, $login_autocomp, $nmgp_tab_label;

      $C_formatado = true;
      $this->Ini->sc_Include($this->Ini->path_lib_php . "/nm_gp_limpa.php", "F", "nm_limpa_valor") ; 
      $this->Ini->sc_Include($this->Ini->path_lib_php . "/nm_conv_dados.php", "F", "nm_conv_limpa_dado") ; 
      $this->Ini->sc_Include($this->Ini->path_lib_php . "/nm_edit.php", "F", "nmgp_Form_Num_Val") ; 
      if (!empty($idnurse_autocomp) && empty($idnurse))
      {
          $idnurse = $idnurse_autocomp;
      }
      if (!empty($namenurse_autocomp) && empty($namenurse))
      {
          $namenurse = $namenurse_autocomp;
      }
      if (!empty($login_autocomp) && empty($login))
      {
          $login = $login_autocomp;
      }
      $idnurse_cond_salva = $idnurse_cond; 
      if (!isset($idnurse_input_2) || $idnurse_input_2 == "")
      {
          $idnurse_input_2 = $idnurse;
      }
      $namenurse_cond_salva = $namenurse_cond; 
      if (!isset($namenurse_input_2) || $namenurse_input_2 == "")
      {
          $namenurse_input_2 = $namenurse;
      }
      ...
      if ($idnurse_cond != "in")
      {
          nm_limpa_numero($idnurse, $_SESSION['scriptcase']['reg_conf']['grup_num']) ; 
      }
      if ($login_cond != "in")
      {
          nm_limpa_numero($login, $_SESSION['scriptcase']['reg_conf']['grup_num']) ; 
      }
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_nurses']['campos_busca']  = array(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_nurses']['campos_busca']['idnurse'] = $idnurse; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_nurses']['campos_busca']['idnurse_cond'] = $idnurse_cond_salva; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_nurses']['dyn_search']  = array(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_nurses']['campos_busca']['namenurse'] = $namenurse; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_nurses']['campos_busca']['namenurse_cond'] = $namenurse_cond_salva; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_nurses']['dyn_search']  = array(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_nurses']['campos_busca']['login'] = $login; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_nurses']['campos_busca']['login_cond'] = $login_cond_salva; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_nurses']['dyn_search']  = array(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_nurses']['campos_busca']['NM_operador'] = $this->NM_operador; 
      if ($this->NM_ajax_flag && $this->NM_ajax_opcao == "ajax_grid_search")
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_nurses']['campos_busca'] = $Temp_Busca;
      }
      if ($idnurse_cond != "in" && $idnurse_cond != "bw" && !empty($idnurse) && !is_numeric($idnurse))
      {
          if (!empty($this->Campos_Mens_erro)){$this->Campos_Mens_erro .= "<br>";}$this->Campos_Mens_erro .= $this->Ini->Nm_lang['lang_errm_ajax_data'] . " : " . $this->Ini->Nm_lang['lang_nurses_fld_idnurse'] . "";
      }
      if ($idnurse_cond == "bw" && ((!empty($idnurse) && !is_numeric($idnurse)) || (!empty($idnurse_input_2) && !is_numeric($idnurse_input_2)) ))
      {
          if (!empty($this->Campos_Mens_erro)){$this->Campos_Mens_erro .= "<br>";}$this->Campos_Mens_erro .= $this->Ini->Nm_lang['lang_errm_ajax_data'] . " : " . $this->Ini->Nm_lang['lang_nurses_fld_idnurse'] . "";
      }
      if ($login_cond != "in" && $login_cond != "bw" && !empty($login) && !is_numeric($login))
      {
          if (!empty($this->Campos_Mens_erro)){$this->Campos_Mens_erro .= "<br>";}$this->Campos_Mens_erro .= $this->Ini->Nm_lang['lang_errm_ajax_data'] . " : " . $this->Ini->Nm_lang['lang_nurses_fld_login'] . "";
      }
      if ($login_cond == "bw" && ((!empty($login) && !is_numeric($login)) || (!empty($login_input_2) && !is_numeric($login_input_2)) ))
      {
          if (!empty($this->Campos_Mens_erro)){$this->Campos_Mens_erro .= "<br>";}$this->Campos_Mens_erro .= $this->Ini->Nm_lang['lang_errm_ajax_data'] . " : " . $this->Ini->Nm_lang['lang_nurses_fld_login'] . "";
      }
      if (!empty($this->Campos_Mens_erro)) 
   ...
   { 
      ...
      if (!empty($nmgp_def_dados) && isset($cmp2) && !empty($cmp2))
      {
          ...
          $this->cmp_formatado['idnurse'] = $cmp2;
      }
      elseif (!empty($nmgp_def_dados) && isset($cmp1) && !empty($cmp1))
      {
          ...
          $this->cmp_formatado['idnurse'] = $cmp1;
      }
      else
      {
          $cmp1 = $idnurse;
    ...
    if ($namenurse != '') {
      ...
      if (!empty($nmgp_def_dados) && isset($cmp2) && !empty($cmp2))
      {
          ...
          $this->cmp_formatado['namenurse'] = $cmp2;
      }
      elseif (!empty($nmgp_def_dados) && isset($cmp1) && !empty($cmp1))
      {
          if ($_SESSION['scriptcase']['charset'] != "UTF-8")
          ...
          $this->cmp_formatado['namenurse'] = $cmp1;
      }
      else
      {
          $this->cmp_formatado['namenurse'] = $namenurse;
      }
      $nmgp_def_dados = array();
    if ($login != '') {
      $login_look = (is_string($login) ? substr($this->Db->qstr($login), 1, -1) : $login); 
   ...
   function processa_campos($Res_comp='pesq')
   {
      ...
      if (isset($idnurse) || $idnurse_cond == "nu" || $idnurse_cond == "nn" || $idnurse_cond == "ep" || $idnurse_cond == "ne")
      {
         $this->monta_condicao("idnurse", $idnurse_cond, $idnurse, "", "idnurse", "INT", false);
      }

      //----- $namenurse
      $this->Date_part = false;
      if (isset($namenurse) || $namenurse_cond == "nu" || $namenurse_cond == "nn" || $namenurse_cond == "ep" || $namenurse_cond == "ne")
      {
         $this->monta_condicao("namenurse", $namenurse_cond, $namenurse, "", "namenurse", "VARCHAR", false);
      }
      ...
      {
         $this->monta_condicao("login", $login_cond, $login, "", "login", "INT", false);
      }
   }
   ...
   function finaliza_resultado()
   {
      ...
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_nurses']['campos_busca']) && $_SESSION['scriptcase']['charset'] != "UTF-8")
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_nurses']['campos_busca'] = NM_conv_charset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_nurses']['campos_busca'], "UTF-8", $_SESSION['scriptcase']['charset']);
      }

      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_nurses']['where_pesq_lookup']  = $this->comando_sum . $this->comando_fim;
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_nurses']['where_pesq']         = $this->comando . $this->comando_fim;
      ...
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['grid_nurses']['where_pesq_filtro'] = " (" . trim($this->comando_filtro) . ")";
      }
         $_SESSION['sc_session'][$this->Ini->sc_page]['grid_nurses']['cond_pesq'] .= $this->NM_operador;
