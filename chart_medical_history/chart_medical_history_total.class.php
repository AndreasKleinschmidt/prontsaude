...
class chart_medical_history_total
{
   var $Db;
   var $Erro;
   var $Ini;
   ...
   function __construct($sc_page)
   {
      ...
      if (isset($_SESSION['sc_session'][$this->sc_page]['chart_medical_history']['campos_busca']) && !empty($_SESSION['sc_session'][$this->sc_page]['chart_medical_history']['campos_busca']))
      { 
   ...
   function Calc_resumo_sc_free_group_by($destino_resumo, $res_export=false, $ind_compara="", $tipo="")
   {
      global $nm_lang;
      $this->nm_data = new nm_data("en_us");
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_medical_history']['sql_tot_res']);
      ...
      $ind_qb                = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_medical_history']['SC_Ind_Groupby'];
      $cmp_sql_def   = array('diagnosis' => "diagnosis");
      $cmps_quebra_atual = array();
      foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_medical_history']['SC_Gb_Free_cmp'] as $cmp => $resto)
      {
          $cmps_quebra_atual[] = $cmp;
      }
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_medical_history']['SC_force_fiels_gb']) && !empty($_SESSION['sc_session'][$this->Ini->sc_page]['chart_medical_history']['SC_force_fiels_gb']))
      {
          $cmps_quebra_atual = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_medical_history']['SC_force_fiels_gb'];
      }
      ...
      {
          if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_medical_history']['SC_clear_total']) && $_SESSION['sc_session'][$this->Ini->sc_page]['chart_medical_history']['SC_clear_total']) {
      ...
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_medical_history']['SC_clear_total']);
      $ind_cmps  = 2;
      $ind_alias = "1";
      $cmp_dim   = array();
      foreach ($cmps_quebra_atual as $cmp_gb)
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['chart_medical_history']['arr_total'] = array();
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          $Arr_tot_name = "array_total_" . $cmp_gb;
          $_SESSION['sc_session'][$this->Ini->sc_page]['chart_medical_history']['arr_total'][$cmp_gb] = $this->$Arr_tot_name;
      }
   }
   ...
   function Res_Totaliza_sc_free_group_by($ind_qb, $cmp_tot, $arr_tots, $group, $join, $cmps_quebras, $cmps_quebras1, $cmps_quebras2, $cmps_quebrasS, $Cmp_dim, $cmps_quebra_atual, $cmp_sql_def, $res_export)
   {
      ...
      $cmp_sql_tp_num = array('idhistory' => 'N','event_idevent' => 'N','patients_idpatient' => 'N','doctor_iddoctor' => 'N','nurses_idnurse' => 'N');
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
         $cmd_simp = "select count(*), count(distinct event_idevent)#@#cmps_quebras#@# from " . $this->Ini->nm_tabela . " " . $where_ok;
         $comando  = "select count(*), count(distinct SC_metric1)#@#cmps_quebras#@# from (";
         $comando .= "select event_idevent as SC_metric1, " . $cmps_quebras1 . " from " . $this->Ini->nm_tabela . " " .  $where_ok . ") SC_sel1 INNER JOIN (";
      ...
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_medical_history']['sql_tot_res']))
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['chart_medical_history']['sql_tot_res'] = str_replace("#@#cmps_quebras#@#", "", $comando);
      }
      $comando  = str_replace("#@#cmps_quebras#@#", "," . $cmps_quebras, $comando);
      $comando .= " group by " . $cmps_quebras . " order by " .  $cmps_quebras;
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_medical_history']['Res_search_metric_use']) || empty($_SESSION['sc_session'][$this->Ini->sc_page]['chart_medical_history']['Res_search_metric_use']))
      {
          $comando = $cmd_simp;
          $cmps_S  = "";
      ...
      $format_dimensions['diagnosis']['reg'] = "S";
      $format_dimensions['diagnosis']['msk'] = "";
      while (!$rt->EOF)
      {
          $sql_where = "";
   ...
   {
      global $nada, $nm_lang ;
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_medical_history']['contr_total_geral'] == "OK") 
      { 
          return; 
      } 
      $_SESSION['sc_session'][$this->Ini->sc_page]['chart_medical_history']['tot_geral'] = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
          $nm_comando = "select count(*), count(distinct event_idevent) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['chart_medical_history']['where_pesq']; 
      } 
      elseif (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_mssql))
      { 
          $nm_comando = "select count(*), count(distinct event_idevent) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['chart_medical_history']['where_pesq']; 
      ...
      if ((isset($this->Ini->nm_bases_vfp) && in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_vfp)) || (isset($this->Ini->nm_bases_odbc) && in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_odbc)))
      {
          $vl_statistic = $this->Calc_statist_manual_sc_free_group_by($_SESSION['sc_session'][$this->Ini->sc_page]['chart_medical_history']['where_pesq']);
          foreach ($vl_statistic as $ind => $val)
          {
              $rt->fields[$ind] = $val;
          }
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['chart_medical_history']['tot_geral'][0] = "" . $this->Ini->Nm_lang['lang_msgs_totl'] . ""; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['chart_medical_history']['tot_geral'][1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $rt->fields[1] = (string)$rt->fields[1]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['chart_medical_history']['tot_geral'][2] = $rt->fields[1]; 
      $rt->Close(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['chart_medical_history']['contr_total_geral'] = "OK";
   } 


   function Calc_statist_manual_sc_free_group_by($sql_where="")
   {
       $comando  = "select event_idevent from " . $this->Ini->nm_tabela . "  " .  $sql_where;
       $_SESSION['scriptcase']['sc_sql_ult_comando'] = $comando;
       $_SESSION['scriptcase']['sc_sql_ult_conexao'] = '';
   ...
   //-----  diagnosis
   function quebra_diagnosis_sc_free_group_by($diagnosis, $Where_qb, $Cmp_Name) 
   {
      ...
      $nm_comando = $this->Ajust_statistic($nm_comando);
      if (empty($_SESSION['sc_session'][$this->Ini->sc_page]['chart_medical_history']['where_pesq'])) 
      { 
      ...
      { 
         $where_ok = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_medical_history']['where_pesq'] . " and " . $Where_qb ; 
      } 
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando;
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = '';
      if (!$rt = $this->Db->Execute($nm_comando)) 
      ...
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_medical_history']['opcao'] == "pdf" && isset($_SESSION['nm_session']['sys_wkhtmltopdf_show_html_content']) && $_SESSION['nm_session']['sys_wkhtmltopdf_show_html_content'] == 'Y') {
          $tot_diagnosis[0] = NM_encode_input(sc_strip_script($diagnosis));
      }
      else {
          $tot_diagnosis[0] = sc_strip_script($diagnosis) ; 
      }
      $tot_diagnosis[1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $tot_diagnosis[2] = (string)$rt->fields[1]; 
      $rt->Close(); 
      $$Var_name_gb = $tot_diagnosis;
   } 
