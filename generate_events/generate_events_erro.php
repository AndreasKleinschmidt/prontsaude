...
$old_error_handler = set_error_handler("generate_events_scriptcase_error_handler");

function generate_events_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe="", $err_usr=false)
{
    ...
    if (isset($_SESSION['scriptcase']['generate_events']['contr_erro']) && $_SESSION['scriptcase']['generate_events']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_generate_events->contr_generate_events->Apl_com_erro = true;
            if ($inicial_generate_events->contr_generate_events->NM_ajax_flag)
            {
                $inicial_generate_events->contr_generate_events->NM_ajax_info['errList']['geral_generate_events'][] = $msg;
                return;
            }
            generate_events_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function generate_events_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['generate_events'])  && '' != $_SESSION['scriptcase']['error_icon']['generate_events'])  ? $_SESSION['scriptcase']['error_icon']['generate_events']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['generate_events']) && '' != $_SESSION['scriptcase']['error_close']['generate_events']) ? $_SESSION['scriptcase']['error_close']['generate_events'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['generate_events']) && '' != $_SESSION['scriptcase']['error_span_title']['generate_events']) ? $_SESSION['scriptcase']['error_span_title']['generate_events'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['generate_events']) && '' != $_SESSION['scriptcase']['error_icon_title']['generate_events']) ? $_SESSION['scriptcase']['error_icon_title']['generate_events'] : '';
?>
...
<?php
    ...
    $_SESSION['scriptcase']['generate_events']['error_buffer'] .= ob_get_flush();
}
