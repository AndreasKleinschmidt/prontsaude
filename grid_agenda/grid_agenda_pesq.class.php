...
class grid_agenda_pesq
{
   ...
   function init()
   {
    ...
    {
      global $doctor_iddoctor_cond, $doctor_iddoctor,
...
if (!isset($this->sc_temp_var_doctor)) {$this->sc_temp_var_doctor = (isset($_SESSION['var_doctor'])) ? $_SESSION['var_doctor'] : "";}
 if( !empty($this->sc_temp_var_doctor) ) { $doctor_iddoctor  = $this->sc_temp_var_doctor; }


$start_date_mes  = date('m'); 
$start_date_dia  = date('d'); 
$start_date_ano  = date('Y');
...
$_SESSION['scriptcase']['grid_agenda']['contr_erro'] = 'off'; 
      ...
      {
          $start_date_dia = $start_date_day; 
      }
      if (isset($start_date_month))
      {
          $start_date_mes = $start_date_month; 
      }
      if (isset($start_date_year))
      {
          $start_date_ano = $start_date_year; 
      }
      if (isset($start_date))
      {
          $start_date = str_replace("0000", "", $start_date);
          $start_date = str_replace("-00", "-", $start_date);
          $start_dateXX = explode("-", $start_date);
          if (isset($start_dateXX[2]))
          {
              $start_date_dia = $start_dateXX[2]; 
          }
          if (isset($start_dateXX[1]))
          {
              $start_date_mes = $start_dateXX[1]; 
          }
          if (isset($start_dateXX[0]))
          {
              $start_date_ano = $start_dateXX[0]; 
          }
      }
      if (isset($start_date_input_2_day))
      {
          $start_date_input_2_dia = $start_date_input_2_day; 
      }
      if (isset($start_date_input_2_month))
      {
          $start_date_input_2_mes = $start_date_input_2_month; 
      }
      if (isset($start_date_input_2_year))
      {
          $start_date_input_2_ano = $start_date_input_2_year; 
      }
      if (isset($start_date_2))
      {
          $start_date_2 = str_replace("0000", "", $start_date_2);
          $start_date_2 = str_replace("-00", "-", $start_date_2);
          $start_dateXX = explode("-", $start_date_2);
          if (isset($start_dateXX[2]))
          {
              $start_date_input_2_dia = $start_dateXX[2]; 
          }
          if (isset($start_dateXX[1]))
          {
              $start_date_input_2_mes = $start_dateXX[1]; 
          }
          if (isset($start_dateXX[0]))
          {
              $start_date_input_2_ano = $start_dateXX[0]; 
          }
      }
    }
   }
...
<HTML>
<HEAD>
 <TITLE><?php echo $this->Ini->Nm_lang['lang_agenda_day'] ?></TITLE>
...
var nmdg_Form = "F1";
 ...
 function nm_save_form(pos)
 {
  ...
  var str_out = "";
  str_out += 'SC_doctor_iddoctor_cond#NMF#' + search_get_sel_txt('SC_doctor_iddoctor_cond') + '@NMF@';
  str_out += 'SC_doctor_iddoctor#NMF#' + search_get_select('SC_doctor_iddoctor') + '@NMF@';
  str_out += 'SC_start_date_cond#NMF#' + search_get_sel_txt('SC_start_date_cond') + '@NMF@';
  opc = search_get_sel_txt('SC_start_date_cond');
  if (opc.substring(0, 3) != "bi_")
  {
...
<div class="scFilterHeader" style="height: 54px; padding: 17px 15px; box-sizing: border-box;margin: -1px 0px 0px 0px;width: 100%;">
    <div class="scFilterHeaderFont" style="float: left; text-transform: uppercase;"><?php echo $this->Ini->Nm_lang['lang_agenda_day'] ?></div>
...
<?php
   ...
   {
      global 
             $doctor_iddoctor_cond, $doctor_iddoctor,
             $start_date_cond, $start_date, $start_date_dia, $start_date_mes, $start_date_ano, $start_date_input_2_dia, $start_date_input_2_mes, $start_date_input_2_ano,
      ...
      { 
          ...
          $doctor_iddoctor = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_agenda']['campos_busca']['doctor_iddoctor']; 
          $doctor_iddoctor_cond = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_agenda']['campos_busca']['doctor_iddoctor_cond']; 
          $start_date_dia = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_agenda']['campos_busca']['start_date_dia']; 
          $start_date_mes = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_agenda']['campos_busca']['start_date_mes']; 
          $start_date_ano = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_agenda']['campos_busca']['start_date_ano']; 
          $start_date_input_2_dia = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_agenda']['campos_busca']['start_date_input_2_dia']; 
          $start_date_input_2_mes = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_agenda']['campos_busca']['start_date_input_2_mes']; 
          $start_date_input_2_ano = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_agenda']['campos_busca']['start_date_input_2_ano']; 
          $start_date_cond = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_agenda']['campos_busca']['start_date_cond']; 
          $this->NM_operador = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_agenda']['campos_busca']['NM_operador']; 
      } 
      if (!isset($doctor_iddoctor_cond) || empty($doctor_iddoctor_cond))
      {
         $doctor_iddoctor_cond = "eq";
      }
      if (!isset($start_date_cond) || empty($start_date_cond))
      {
         $start_date_cond = "eq";
      }
      if (isset($start_date_cond) && substr($start_date_cond, 0, 3) == "bi_")
      {
         $Temp_cond = $start_date_cond;
         $this->Ini->process_cond_bi($Temp_cond, $BI_data1, $BI_data2);
         $start_date_dia = substr($BI_data1, 0, 2);
         $start_date_mes = substr($BI_data1, 2, 2);
         $start_date_ano = substr($BI_data1, 4, 4);
         if (strlen($BI_data1) > 8)
         {
             $start_date_hor = substr($BI_data1, 8, 2);
             $start_date_min = substr($BI_data1, 10, 2);
             $start_date_seg = substr($BI_data1, 12, 2);
         }
         $start_date_input_2_dia = substr($BI_data2, 0, 2);
         $start_date_input_2_mes = substr($BI_data2, 2, 2);
         $start_date_input_2_ano = substr($BI_data2, 4, 4);
         if (strlen($BI_data2) > 8)
         {
             $start_date_input_2_hor = substr($BI_data2, 8, 2);
             $start_date_input_2_min = substr($BI_data2, 10, 2);
             $start_date_input_2_seg = substr($BI_data2, 12, 2);
         }
         $Script_BI .= "  formata_bi_start_date('" . $start_date_cond . "', '" . $Temp_cond . "', '" . $BI_data1 . "');\r\n";
      }
      $display_aberto  = "style=display:";
      $display_fechado = "style=display:none";
      $opc_hide_input = array("nu","nn","ep","ne");
      $str_hide_doctor_iddoctor = (in_array($doctor_iddoctor_cond, $opc_hide_input)) ? $display_fechado : $display_aberto;
      $str_hide_start_date = (in_array($start_date_cond, $opc_hide_input)) ? $display_fechado : $display_aberto;

      $str_display_doctor_iddoctor = ('bw' == $doctor_iddoctor_cond) ? $display_aberto : $display_fechado;
      $str_display_start_date = ('bw' == $start_date_cond || 'bi_' == substr($start_date_cond, 0, 3)) ? $display_aberto : $display_fechado;

      if (!isset($doctor_iddoctor) || $doctor_iddoctor == "")
      {
          $doctor_iddoctor = "";
      }
      if (isset($doctor_iddoctor) && !empty($doctor_iddoctor))
      {
         $tmp_pos = (is_string($doctor_iddoctor)) ? strpos($doctor_iddoctor, "##@@") : false;
         if ($tmp_pos === false)
         { }
         else
         {
         $doctor_iddoctor = substr($doctor_iddoctor, 0, $tmp_pos);
         }
      }
...
?>
 ...
 <TR align="center">
  ...
  <TD width="100%" height="">
   ...
   <tr valign="top">
      ...
      <TD id='SC_doctor_iddoctor_label' class="scFilterLabelOdd"><?php echo (isset($this->New_label['doctor_iddoctor'])) ? $this->New_label['doctor_iddoctor'] : "" . $this->Ini->Nm_lang['lang_event_fld_doctor_iddoctor'] . ""; ?></TD>
      
      <INPUT type="hidden" id="SC_doctor_iddoctor_cond" name="doctor_iddoctor_cond" value="eq">
 
     <TD colspan=2 class="scFilterFieldOdd">
      <TABLE  border="0" cellpadding="0" cellspacing="0">
       <TR id="id_hide_doctor_iddoctor" <?php echo $str_hide_doctor_iddoctor?> valign="top">
 ...
 $nmgp_tab_label .= "doctor_iddoctor?#?" . $SC_Label . "?@?";
 $date_sep_bw = " ";
...
<?php
      $doctor_iddoctor_look = (is_string($doctor_iddoctor) ? substr($this->Db->qstr($doctor_iddoctor), 1, -1) : $doctor_iddoctor); 
      $nmgp_def_dados = "" ; 
      $nm_comando = "SELECT iddoctor, namedoctor  FROM doctors  ORDER BY namedoctor"; 
      ...
      { 
         $_SESSION['sc_session'][$this->Ini->sc_page]['grid_agenda']['psq_check_ret']['doctor_iddoctor'] = array();
         while (!$rs->EOF) 
         { 
            $_SESSION['sc_session'][$this->Ini->sc_page]['grid_agenda']['psq_check_ret']['doctor_iddoctor'][] = trim($rs->fields[0]);
...
?>
   <span id="idAjaxSelect_doctor_iddoctor">
      <SELECT class="scFilterObjectOdd" id="SC_doctor_iddoctor" name="doctor_iddoctor"  size="1">
<?php
      ...
      foreach ($nm_opcoes as $nm_opcao)
      {
         if (!empty($nm_opcao))
         {
            ...
            if ("" != $doctor_iddoctor)
            {
                    $doctor_iddoctor_sel = ($nm_opc_cod === $doctor_iddoctor) ? "selected" : "";
            }
            else
            {
               $doctor_iddoctor_sel = ("S" == $nm_opc_sel) ? "selected" : "";
            }
            $nm_sc_valor = $nm_opc_val;
            $nm_opc_val = $nm_sc_valor;
?>
       <OPTION value="<?php echo NM_encode_input($nm_opc_cod . $delimitador . $nm_opc_val); ?>" <?php echo $doctor_iddoctor_sel; ?>><?php echo $nm_opc_val; ?></OPTION>
<?php
         }
      }
?>
...
?>
      ...
      <TD id='SC_start_date_label' class="scFilterLabelEven"><?php echo (isset($this->New_label['start_date'])) ? $this->New_label['start_date'] : "" . $this->Ini->Nm_lang['lang_event_fld_start_date'] . ""; ?></TD>
     <TD class="scFilterFieldEven"> 
      <SELECT class="scFilterObjectEven" id="SC_start_date_cond" name="start_date_cond" onChange="nm_campos_between(document.getElementById('id_vis_start_date'), this, 'start_date')">
       ...
       <OPTION value="bi_TP" <?php if ("bi_TP" == $start_date_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_ever'] ?></OPTION>
       <OPTION value="bi_HJ" <?php if ("bi_HJ" == $start_date_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_tday'] ?></OPTION>
       <OPTION value="bi_OT" <?php if ("bi_OT" == $start_date_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_yest'] ?></OPTION>
       <OPTION value="bi_U7" <?php if ("bi_U7" == $start_date_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_lst7'] ?></OPTION>
       <OPTION value="bi_US" <?php if ("bi_US" == $start_date_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_lstw_bsnd'] ?></OPTION>
       <OPTION value="bi_MM" <?php if ("bi_MM" == $start_date_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_this_mnth'] ?></OPTION>
       <OPTION value="bi_UM" <?php if ("bi_UM" == $start_date_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_last_mnth'] ?></OPTION>
       <OPTION value="bi_AM" <?php if ("bi_AM" == $start_date_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_tomw'] ?></OPTION>
       <OPTION value="bi_PS" <?php if ("bi_PS" == $start_date_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_nxtw_mond_frid'] ?></OPTION>
       <OPTION value="bi_P3" <?php if ("bi_P3" == $start_date_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_nx30'] ?></OPTION>
       <OPTION value="bi_PM" <?php if ("bi_PM" == $start_date_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_next_mnth'] ?></OPTION>
       <OPTION value="bi_P7" <?php if ("bi_P7" == $start_date_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_nxt7'] ?></OPTION>
       <OPTION value="bi_M6" <?php if ("bi_M6" == $start_date_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_last_06mo'] ?></OPTION>
       <OPTION value="bi_M3" <?php if ("bi_M3" == $start_date_cond) { echo "selected"; } ?>><?php echo $this->Ini->Nm_lang['lang_srch_last_03mo'] ?></OPTION>
      </SELECT>
       </TD>
     <TD  class="scFilterFieldEven"><SPAN id="Nm_bi_dados_start_date" style="display:none"></SPAN>
      <TABLE  id="opc_bi_TP_start_date"  style=display:  border="0" cellpadding="0" cellspacing="0">
...
?>
 ...
 function limpa_form()
 {
   ...
   document.F1.doctor_iddoctor_cond.value = 'eq';
   nm_campos_between(document.getElementById('id_vis_doctor_iddoctor'), document.F1.doctor_iddoctor_cond, 'doctor_iddoctor');
   document.F1.doctor_iddoctor.value = "";
 ...
 {
    if (Field == 'all' || Field == 'doctor_iddoctor') {
       Sc_carga_select2_doctor_iddoctor();
    }
 }
 function Sc_carga_select2_doctor_iddoctor()
 {
  $("#SC_doctor_iddoctor").select2(
    {
      language: {
        noResults: function() {
 ...
 {
    $('#SC_start_date_dia').bind('change', function() {sc_grid_agenda_valida_dia(this)});
    $('#SC_start_date_input_2_dia').bind('change', function() {sc_grid_agenda_valida_dia(this)});
    $('#SC_start_date_input_2_mes').bind('change', function() {sc_grid_agenda_valida_mes(this)});
    $('#SC_start_date_mes').bind('change', function() {sc_grid_agenda_valida_mes(this)});
 }
...
function formata_bi_start_date(opc, opc_bi, dt_fmt)
{
   if (opc.substring(0,3) != "bi_")
   {
       document.getElementById('Nm_bi_dados_start_date').style.display = 'none';
       document.getElementById('opc_bi_TP_start_date').style.display = '';
       return;
   }
   if (opc == "bi_TP")
   {
       document.getElementById('Nm_bi_dados_start_date').style.display = 'none';
       document.getElementById('opc_bi_TP_start_date').style.display = 'none';
       return;
   }
<?php
   $date_format_show = "";
   ...
   {
       $date_format_show .= (!$Prim) ? " + '" . $_SESSION['scriptcase']['reg_conf']['date_sep'] . "' + ": "";
       $date_format_show .= ($Cada_d == "d") ? "document.F1.start_date_dia.value" : "";
       $date_format_show .= ($Cada_d == "m") ? "document.F1.start_date_mes.value" : "";
       $date_format_show .= ($Cada_d == "Y") ? "document.F1.start_date_ano.value" : "";
       $Prim = false;
   }
?> 
...
<?php
   ...
   {
       $date_format_show .= (!$Prim) ? " + '" . $_SESSION['scriptcase']['reg_conf']['date_sep'] . "' + ": "";
       $date_format_show .= ($Cada_d == "d") ? "document.F1.start_date_input_2_dia.value" : "";
       $date_format_show .= ($Cada_d == "m") ? "document.F1.start_date_input_2_mes.value" : "";
       $date_format_show .= ($Cada_d == "Y") ? "document.F1.start_date_input_2_ano.value" : "";
       $Prim = false;
   }
?> 
       saida += <?php echo $date_format_show ?>;
   }
   document.getElementById('Nm_bi_dados_start_date').innerHTML = saida;
   document.getElementById('opc_bi_TP_start_date').style.display = 'none';
   document.getElementById('Nm_bi_dados_start_date').style.display = '';
}
<?php
  echo $Script_BI;
?>
...
<?php
   ...
   function gera_array_filtros()
   {
       ...
       $NM_patch   = "ProntSaude/grid_agenda";
   ...
   function recupera_filtro($NM_filters, $Orig_Fil)
   {
      ...
      $tp_fields['SC_doctor_iddoctor_cond'] = 'cond';
      $tp_fields['SC_doctor_iddoctor'] = 'select';
      ...
      if (is_file($NM_patch))
      {
          ...
          foreach ($NMcmp_filter as $Cada_cmp)
          {
              ...
              else
              {
                  ...
                  if ($Cada_cmp[0] == 'SC_start_date_cond')
                  {
                      $opc = $Cada_cmp[1];
   ...
   function trata_campos()
   {
      ...
      $doctor_iddoctor_cond_salva = $doctor_iddoctor_cond; 
      if (!isset($doctor_iddoctor_input_2) || $doctor_iddoctor_input_2 == "")
      {
          $doctor_iddoctor_input_2 = $doctor_iddoctor;
      }
      ...
      $tmp_pos = (is_string($doctor_iddoctor)) ? strpos($doctor_iddoctor, "##@@") : false;
      if ($tmp_pos === false) {
          $L_lookup = $doctor_iddoctor;
      }
      else {
          $L_lookup = substr($doctor_iddoctor, 0, $tmp_pos);
      }
      if ($this->NM_ajax_opcao != "ajax_grid_search_change_fil" && !empty($L_lookup) && !in_array($L_lookup, $_SESSION['sc_session'][$this->Ini->sc_page]['grid_agenda']['psq_check_ret']['doctor_iddoctor'])) {
          if (!empty($this->Campos_Mens_erro)) {$this->Campos_Mens_erro .= "<br>";}$this->Campos_Mens_erro .= "" . $this->Ini->Nm_lang['lang_event_fld_doctor_iddoctor'] . " : " . $this->Ini->Nm_lang['lang_errm_ajax_data'];
      }
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_agenda']['campos_busca']  = array(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_agenda']['campos_busca']['doctor_iddoctor'] = $doctor_iddoctor; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_agenda']['campos_busca']['doctor_iddoctor_cond'] = $doctor_iddoctor_cond_salva; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_agenda']['dyn_search']  = array(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_agenda']['campos_busca']['start_date_dia'] = $start_date_dia; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_agenda']['campos_busca']['start_date_mes'] = $start_date_mes; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_agenda']['campos_busca']['start_date_ano'] = $start_date_ano; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_agenda']['campos_busca']['start_date_input_2_dia'] = $start_date_input_2_dia; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_agenda']['campos_busca']['start_date_input_2_mes'] = $start_date_input_2_mes; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_agenda']['campos_busca']['start_date_input_2_ano'] = $start_date_input_2_ano; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_agenda']['campos_busca']['start_date_cond'] = $start_date_cond_salva; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_agenda']['dyn_search']  = array(); 
      ...
      $doctor_iddoctor_LKP = $doctor_iddoctor;
      $Tmp_pos = (is_string($doctor_iddoctor)) ? strpos($doctor_iddoctor, "##@@") : false;
      if ($Tmp_pos !== false)
      {
          $doctor_iddoctor = substr($doctor_iddoctor, 0, $Tmp_pos);
      ...
      $start_date_day   = $start_date_dia; 
      $start_date_month = $start_date_mes; 
      $start_date_year  = $start_date_ano; 
      $start_date_diaSv = $start_date_dia; 
      $start_date_mesSv = $start_date_mes; 
      $start_date_anoSv = $start_date_ano; 
      $start_date  = str_repeat(0, (4 - strlen($start_date_ano))) . $start_date_ano . "-"; 
      $start_date .= str_repeat(0, (2 - strlen($start_date_mes))) . $start_date_mes . "-"; 
      $start_date .= str_repeat(0, (2 - strlen($start_date_dia))) . $start_date_dia; 
      $start_date_input_2_day   = $start_date_input_2_dia; 
      $start_date_input_2_month = $start_date_input_2_mes; 
      $start_date_input_2_year  = $start_date_input_2_ano; 
      $start_date_input_2_diaSv = $start_date_input_2_dia; 
      $start_date_input_2_mesSv = $start_date_input_2_mes; 
      $start_date_input_2_anoSv = $start_date_input_2_ano; 
      $start_date_2  = str_repeat(0, (4 - strlen($start_date_input_2_ano))) . $start_date_input_2_ano . "-"; 
      $start_date_2 .= str_repeat(0, (2 - strlen($start_date_input_2_mes))) . $start_date_input_2_mes . "-"; 
      $start_date_2 .= str_repeat(0, (2 - strlen($start_date_input_2_dia))) . $start_date_input_2_dia; 
      $_SESSION['scriptcase']['grid_agenda']['contr_erro'] = 'on';
...
if (!isset($this->sc_temp_doc_id)) {$this->sc_temp_doc_id = (isset($_SESSION['doc_id'])) ? $_SESSION['doc_id'] : "";}
 $this->sc_temp_doc_id = $doctor_iddoctor ;
if (isset($this->sc_temp_doc_id)) {$_SESSION['doc_id'] = $this->sc_temp_doc_id;}
$_SESSION['scriptcase']['grid_agenda']['contr_erro'] = 'off'; 
      if ($doctor_iddoctor_LKP == $doctor_iddoctor)
      {
          $doctor_iddoctor = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_agenda']['campos_busca']['doctor_iddoctor'];
      }
      if ($start_date_day != $start_date_diaSv)
      {
          $start_date_dia = $start_date_day; 
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_agenda']['campos_busca']['start_date_dia'] = $start_date_dia; 
      }
      if ($start_date_month != $start_date_mesSv)
      {
          $start_date_mes = $start_date_month; 
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_agenda']['campos_busca']['start_date_mes'] = $start_date_mes; 
      }
      if ($start_date_year != $start_date_anoSv)
      {
          $start_date_ano = $start_date_year; 
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_agenda']['campos_busca']['start_date_ano'] = $start_date_ano; 
      }
      ...
      $start_date = str_replace("-00", "-", $start_date);
      $start_dateXX = explode("-", $start_date);
      if (isset($start_dateXX[2]) && $start_dateXX[2] != $start_date_diaSv)
      {
          $start_date_dia = $start_dateXX[2]; 
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_agenda']['campos_busca']['start_date_dia'] = $start_dateXX[2]; 
      }
      if (isset($start_dateXX[1]) && $start_dateXX[1] != $start_date_mesSv)
      {
          $start_date_mes = $start_dateXX[1]; 
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_agenda']['campos_busca']['start_date_mes'] = $start_dateXX[1]; 
      }
      if (isset($start_dateXX[0]) && $start_dateXX[0] != $start_date_anoSv)
      {
          $start_date_ano = $start_dateXX[0]; 
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_agenda']['campos_busca']['start_date_ano'] = $start_dateXX[0]; 
      }
      if ($start_date_input_2_day != $start_date_input_2_diaSv)
      {
          $start_date_input_2_dia = $start_date_input_2_day; 
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_agenda']['campos_busca']['start_date_input_2_dia'] = $start_date_input_2_dia; 
      }
      if ($start_date_input_2_month != $start_date_input_2_mesSv)
      {
          $start_date_input_2_mes = $start_date_input_2_month; 
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_agenda']['campos_busca']['start_date_input_2_mes'] = $start_date_input_2_mes; 
      }
      if ($start_date_input_2_year != $start_date_input_2_anoSv)
      {
          $start_date_input_2_ano = $start_date_input_2_year; 
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_agenda']['campos_busca']['start_date_input_2_ano'] = $start_date_input_2_ano; 
      }
      ...
      $start_date_2 = str_replace("-00", "-", $start_date_2);
      $start_dateXX = explode("-", $start_date_2);
      if (isset($start_dateXX[2]) && $start_dateXX[2] != $start_date_input_2_diaSv)
      {
          $start_date_input_2_dia = $start_dateXX[2]; 
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_agenda']['campos_busca']['start_date_input_2_dia'] = $start_dateXX[2]; 
      }
      if (isset($start_dateXX[1]) && $start_dateXX[1] != $start_date_input_2_mesSv)
      {
          $start_date_input_2_mes = $start_dateXX[1]; 
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_agenda']['campos_busca']['start_date_input_2_mes'] = $start_dateXX[1]; 
      }
      if (isset($start_dateXX[0]) && $start_dateXX[0] != $start_date_input_2_anoSv)
      {
          $start_date_input_2_ano = $start_dateXX[0]; 
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_agenda']['campos_busca']['start_date_input_2_ano'] = $start_dateXX[0]; 
      }
      if (!empty($this->Campos_Mens_erro)) 
      {
          return;
      }
      $Conteudo = $doctor_iddoctor;
      if (strpos($Conteudo, "##@@") !== false)
      {
          $Conteudo = substr($Conteudo, strpos($Conteudo, "##@@") + 4);
      }
      $this->cmp_formatado['doctor_iddoctor'] = $Conteudo;
   }
   ...
   function processa_campos($Res_comp='pesq')
   {
      ...
      if (isset($doctor_iddoctor))
      {
         $this->monta_condicao("doctor_iddoctor", $doctor_iddoctor_cond, $doctor_iddoctor, "", "doctor_iddoctor", "INT", false);
      }

      //----- $start_date
      $this->Date_part = false;
      if ($start_date_cond != "bi_TP" && $Res_comp != 'calend')
      {
          ...
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_agenda']['campos_busca']['start_date'] = $val[0];
          ...
          if ($start_date_cond == "BW")
          {
              ...
              $_SESSION['sc_session'][$this->Ini->sc_page]['grid_agenda']['campos_busca']['start_date_input_2'] = $val[1];
   ...
   function css_obj_select_ajax($Obj)
   {
      switch ($Obj)
      {
         case "doctor_iddoctor" : return ('class="scFilterObjectOdd"'); break;
         default       : return ("");
      }
   }
