...
class grid_agenda_res_csv
{
   ...
   function grava_arquivo()
   {
      ...
      if (1 >= sizeof($this->comp_y_axys))
      {
        ...
        if ('S' == $this->Label_CSV) {
            foreach ($_SESSION['sc_session'] [$this->Ini->sc_page] ['grid_agenda'] ['export_info'] ['ordered_x_matrix'] as $rowNumber => $rowInfo) {
            }
        }

        foreach ($_SESSION['sc_session'] [$this->Ini->sc_page] ['grid_agenda'] ['export_data'] as $exportRowInfo) {
            $csvLine = [];
            ...
            for ($i = $dimensionCount; $i < $_SESSION['sc_session'] [$this->Ini->sc_page] ['grid_agenda'] ['export_info'] ['dimension_count'] ['y']; $i++) {
