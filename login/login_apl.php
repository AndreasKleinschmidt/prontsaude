...
//----- 
   function ini_controle()
   {
      ...
      if (isset($this->usr_group_desc) && isset($this->NM_contr_var_session) && $this->NM_contr_var_session == "Yes") 
      {
          $_SESSION['usr_group_desc'] = $this->usr_group_desc;
      }
      ...
      if (isset($_POST["usr_group_desc"]) && isset($this->usr_group_desc)) 
      {
          $_SESSION['usr_group_desc'] = $this->usr_group_desc;
      }
      ...
      if (isset($_GET["usr_group_desc"]) && isset($this->usr_group_desc)) 
      {
          $_SESSION['usr_group_desc'] = $this->usr_group_desc;
      }
...
if (!isset($this->sc_temp_usr_group_desc)) {$this->sc_temp_usr_group_desc = (isset($_SESSION['usr_group_desc'])) ? $_SESSION['usr_group_desc'] : "";}
...
if (!isset($this->sc_temp_usr_login)) {$this->sc_temp_usr_login = (isset($_SESSION['usr_login'])) ? $_SESSION['usr_login'] : "";}
 ...
 unset($_SESSION['usr_group_id']);
 unset($this->sc_temp_usr_group_id);
 unset($_SESSION['usr_group_desc']);
 unset($this->sc_temp_usr_group_desc);
 unset($_SESSION['var_doctor']);
 unset($this->sc_temp_var_doctor);
;
...
switch($this->Ini->sc_get_language())
{
	case "pt_br":
	$this->sc_temp_lang = "_pt";
	break;
	case "es":
	$this->sc_temp_lang = "_es";
	break;
	default:
	$this->sc_temp_lang = "";
	break;
...
if (isset($this->sc_temp_usr_group_desc)) { $_SESSION['usr_group_desc'] = $this->sc_temp_usr_group_desc;}
...
else if($this->rs[0][1] == 'Y')
{
	...
	$usr_group_id	= $this->rs[0][4];
	$usr_group_desc = $this->rs[0][5];
...
;
	 if (isset($usr_group_id)) {$this->sc_temp_usr_group_id = $usr_group_id;}
;
	 if (isset($usr_group_desc)) {$this->sc_temp_usr_group_desc = $usr_group_desc;}
; 
	
	if ($usr_group_id == 2) { 
		 
      $nm_select = "SELECT iddoctor FROM doctors WHERE login = $slogin"; 
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_select; 
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = ''; 
      $this->rs_doc = array();
      if ($SCrx = $this->Db->Execute($nm_select)) 
      { 
          ...
          { 
                 ...
                 for ($SCx = 0; $SCx < $nm_count; $SCx++)
                 { 
                      $this->rs_doc[$SCy] [$SCx] = $SCrx->fields[$SCx];
                 }
                 $SCy++; 
                 $SCrx->MoveNext();
          } 
      ...
      { 
          $this->rs_doc = false;
          $this->rs_doc_erro = $this->Db->ErrorMsg();
      } 

		$var_doctor = $this->rs_doc[0][0];
		 if (isset($var_doctor)) {$this->sc_temp_var_doctor = $var_doctor;}
;
	}
}
else
