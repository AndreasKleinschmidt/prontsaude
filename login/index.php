...
//
   ...
   $_SESSION['scriptcase']['login']['glo_nm_perfil']          = "conn_example";
...
//
   function init()
   {
      ...
      $this->nm_dt_criacao   = "20180227"; 
      $this->nm_hr_criacao   = "091756"; 
...
<?php
      ...
      if (!isset($_SESSION['sc_session'][$this->sc_page]['login']['dashboard_info']['under_dashboard']))
      {
          ...
          $_SESSION['sc_session'][$this->sc_page]['login']['dashboard_info']['remove_background'] = false;
      }
      if (isset($_GET['under_dashboard']) && 1 == $_GET['under_dashboard'])
      {
          if (isset($_GET['own_widget']) && 'dbifrm_widget' == substr($_GET['own_widget'], 0, 13)) {
              ...
              if (isset($_GET['remove_background'])) {
                  $_SESSION['sc_session'][$this->sc_page]['login']['dashboard_info']['remove_background'] = 1 == $_GET['remove_background'];
              }
          }
      }
      elseif (isset($under_dashboard) && 1 == $under_dashboard)
      {
          if (isset($own_widget) && 'dbifrm_widget' == substr($own_widget, 0, 13)) {
              ...
              if (isset($remove_background)) {
                  $_SESSION['sc_session'][$this->sc_page]['login']['dashboard_info']['remove_background'] = 1 == $remove_background;
              }
          }
      }
      ...
      if ($_SESSION['sc_session'][$this->sc_page]['login']['dashboard_info']['under_dashboard'])
      {
        ...
        if (isset($link_remove_background) && 'ok' == $link_remove_background) {
            ...
            $_SESSION['sc_session'][$this->sc_page]['login']['link_info']['remove_background'] = true;
        }
        if (isset($link_margin_top) && '' != $link_margin_top) {
            if (!isset($_SESSION['sc_session'][$this->sc_page]['login']['link_info'])) {
                $_SESSION['sc_session'][$this->sc_page]['login']['link_info'] = array();
            }
            $_SESSION['sc_session'][$this->sc_page]['login']['link_info']['margin_top'] = $link_margin_top;
        }
...
// 
  function conectDB()
  {
      ...
      { 
          $this->Db = db_conect_devel($_SESSION['scriptcase']['login']['glo_nm_conexao'], $this->root . $this->path_prod, 'ProntSaude', 1, $this->force_db_utf8); 
      } 
      else 
      { 
  ...
  function setConnectionHash() {
    if (isset($_SESSION['scriptcase']['nm_sc_retorno']) && !empty($_SESSION['scriptcase']['nm_sc_retorno']) && isset($_SESSION['scriptcase']['login']['glo_nm_conexao']) && !empty($_SESSION['scriptcase']['login']['glo_nm_conexao'])) {
      list($connectionDbms, $connectionHost, $connectionUser, $connectionPassword, $connectionDatabase) = db_conect_devel($_SESSION['scriptcase']['login']['glo_nm_conexao'], $this->root . $this->path_prod, 'ProntSaude', 1, $this->force_db_utf8);
    }
    else {
...
//
    ...
    if ($Sem_Session && (!isset($nmgp_start) || $nmgp_start != "SC")) {
        ...
        elseif (is_file($root . $_SESSION['scriptcase']['login']['glo_nm_path_imag_temp'] . "/sc_apl_default_ProntSaude.txt")) {
            $apl_def = explode(",", file_get_contents($root . $_SESSION['scriptcase']['login']['glo_nm_path_imag_temp'] . "/sc_apl_default_ProntSaude.txt"));
        }
        if (isset($apl_def)) {
            if ($apl_def[0] != "login") {
            ...
            if (isset($_COOKIE['sc_actual_lang_ProntSaude'])) {
                $_SESSION['scriptcase']['login']['session_timeout']['lang'] = $_COOKIE['sc_actual_lang_ProntSaude'];
            }
        }
    }
    ...
    { 
        ...
        if (isset($usr_group_desc)) 
        {
            $_SESSION['usr_group_desc'] = $usr_group_desc;
        }
        if (isset($var_doctor)) 
        {
    ...
    else
    {
        ...
        if (isset($_POST["lang"])) 
        {
            ...
            nm_limpa_str_login($_SESSION['lang']);
        }
        if (isset($_GET["lang"])) 
        {
        ...
        if (isset($_POST["usr_group_id"])) 
        {
            ...
            nm_limpa_str_login($_SESSION['usr_group_id']);
        }
        ...
        if (isset($_POST["usr_group_desc"])) 
        {
            $_SESSION['usr_group_desc'] = $_POST["usr_group_desc"];
            nm_limpa_str_login($_SESSION['usr_group_desc']);
        }
        if (isset($_GET["usr_group_desc"])) 
        {
            $_SESSION['usr_group_desc'] = $_GET["usr_group_desc"];
            nm_limpa_str_login($_SESSION['usr_group_desc']);
        }
        if (isset($_POST["var_doctor"])) 
        {
            $_SESSION['var_doctor'] = $_POST["var_doctor"];
            nm_limpa_str_login($_SESSION['var_doctor']);
        }
...
//
    ...
    function ajax_login_submit_form($login, $pswd, $nm_form_submit, $nmgp_url_saida, $nmgp_opcao, $nmgp_ancora, $nmgp_num_form, $nmgp_parms, $script_case_init, $csrf_token, $bok)
    {
        global $inicial_login;
        //register_shutdown_function("login_pack_ajax_response");
        $inicial_login->contr_login->NM_ajax_flag          = true;
