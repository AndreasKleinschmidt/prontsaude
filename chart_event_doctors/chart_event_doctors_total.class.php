...
class chart_event_doctors_total
{
   var $Db;
   var $Erro;
   var $Ini;
   ...
   function __construct($sc_page)
   {
      ...
      if (isset($_SESSION['sc_session'][$this->sc_page]['chart_event_doctors']['campos_busca']) && !empty($_SESSION['sc_session'][$this->sc_page]['chart_event_doctors']['campos_busca']))
      { 
   ...
   function Calc_resumo_sc_free_group_by($destino_resumo, $res_export=false, $ind_compara="", $tipo="")
   {
      global $nm_lang;
      $this->nm_data = new nm_data("en_us");
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_event_doctors']['sql_tot_res']);
      ...
      $ind_qb                = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_event_doctors']['SC_Ind_Groupby'];
      $cmp_sql_def   = array('start_date' => "start_date",'doctor_iddoctor' => "doctor_iddoctor");
      $cmps_quebra_atual = array();
      foreach ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_event_doctors']['SC_Gb_Free_cmp'] as $cmp => $resto)
      {
          $cmps_quebra_atual[] = $cmp;
      }
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_event_doctors']['SC_force_fiels_gb']) && !empty($_SESSION['sc_session'][$this->Ini->sc_page]['chart_event_doctors']['SC_force_fiels_gb']))
      {
          $cmps_quebra_atual = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_event_doctors']['SC_force_fiels_gb'];
      }
      ...
      {
          if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_event_doctors']['SC_clear_total']) && $_SESSION['sc_session'][$this->Ini->sc_page]['chart_event_doctors']['SC_clear_total']) {
      ...
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_event_doctors']['SC_clear_total']);
      $ind_cmps  = 2;
      $ind_alias = "1";
      $cmp_dim   = array();
      foreach ($cmps_quebra_atual as $cmp_gb)
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['chart_event_doctors']['arr_total'] = array();
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          $Arr_tot_name = "array_total_" . $cmp_gb;
          $_SESSION['sc_session'][$this->Ini->sc_page]['chart_event_doctors']['arr_total'][$cmp_gb] = $this->$Arr_tot_name;
      }
   }
   ...
   function Res_Totaliza_sc_free_group_by($ind_qb, $cmp_tot, $arr_tots, $group, $join, $cmps_quebras, $cmps_quebras1, $cmps_quebras2, $cmps_quebrasS, $Cmp_dim, $cmps_quebra_atual, $cmp_sql_def, $res_export)
   {
      ...
      $cmp_sql_tp_num = array('idevent' => 'N','categ' => 'N','doctor_iddoctor' => 'N','patients_idpatient' => 'N','event_period' => 'N','idevent_status' => 'N','idevent_priority' => 'N');
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
         $cmd_simp = "select count(*), count(distinct idevent)#@#cmps_quebras#@# from " . $this->Ini->nm_tabela . " " . $where_ok;
         $comando  = "select count(*), count(distinct SC_metric1)#@#cmps_quebras#@# from (";
         $comando .= "select idevent as SC_metric1, " . $cmps_quebras1 . " from " . $this->Ini->nm_tabela . " " .  $where_ok . ") SC_sel1 INNER JOIN (";
      ...
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_event_doctors']['sql_tot_res']))
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['chart_event_doctors']['sql_tot_res'] = str_replace("#@#cmps_quebras#@#", "", $comando);
      }
      $comando  = str_replace("#@#cmps_quebras#@#", "," . $cmps_quebras, $comando);
      $comando .= " group by " . $cmps_quebras . " order by " .  $cmps_quebras;
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['chart_event_doctors']['Res_search_metric_use']) || empty($_SESSION['sc_session'][$this->Ini->sc_page]['chart_event_doctors']['Res_search_metric_use']))
      {
          $comando = $cmd_simp;
          $cmps_S  = "";
      ...
      $format_dimensions['start_date']['reg'] = "N";
      $format_dimensions['start_date']['msk'] = "F/Y";
      $format_dimensions['doctor_iddoctor']['reg'] = "S";
      $format_dimensions['doctor_iddoctor']['msk'] = "";
      while (!$rt->EOF)
      {
          $sql_where = "";
          foreach ($Cmp_dim as $Cada_dim => $Ind_sql)
          {
              ...
              if ($Cada_dim == "doctor_iddoctor") {
                  $this->Lookup->lookup_sc_free_group_by_doctor_iddoctor($$Cada_dim,  $doctor_iddoctor);
              }
              if (null === $$Cada_dim)
              {
                  $$Cada_dim = '';
   ...
   {
      global $nada, $nm_lang ;
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['chart_event_doctors']['contr_total_geral'] == "OK") 
      { 
          return; 
      } 
      $_SESSION['sc_session'][$this->Ini->sc_page]['chart_event_doctors']['tot_geral'] = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
          $nm_comando = "select count(*), count(distinct idevent) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['chart_event_doctors']['where_pesq']; 
      } 
      elseif (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_mssql))
      { 
          $nm_comando = "select count(*), count(distinct idevent) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['chart_event_doctors']['where_pesq']; 
      ...
      if ((isset($this->Ini->nm_bases_vfp) && in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_vfp)) || (isset($this->Ini->nm_bases_odbc) && in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_odbc)))
      {
          $vl_statistic = $this->Calc_statist_manual_sc_free_group_by($_SESSION['sc_session'][$this->Ini->sc_page]['chart_event_doctors']['where_pesq']);
          foreach ($vl_statistic as $ind => $val)
          {
              $rt->fields[$ind] = $val;
          }
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['chart_event_doctors']['tot_geral'][0] = "" . $this->Ini->Nm_lang['lang_msgs_totl'] . ""; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['chart_event_doctors']['tot_geral'][1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $rt->fields[1] = (string)$rt->fields[1]; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['chart_event_doctors']['tot_geral'][2] = $rt->fields[1]; 
      $rt->Close(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['chart_event_doctors']['contr_total_geral'] = "OK";
   } 


   function Calc_statist_manual_sc_free_group_by($sql_where="")
   {
       $comando  = "select idevent from " . $this->Ini->nm_tabela . "  " .  $sql_where;
       $_SESSION['scriptcase']['sc_sql_ult_comando'] = $comando;
       $_SESSION['scriptcase']['sc_sql_ult_conexao'] = '';
   ...
   //-----  start_date
   function quebra_start_date_sc_free_group_by($start_date, $Where_qb, $Cmp_Name) 
   {
      $Var_name_gb = "SC_tot_" . $Cmp_Name;
      global $$Var_name_gb;
      $tot_start_date = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
          $nm_comando = "select count(*), count(distinct idevent) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['chart_event_doctors']['where_pesq']; 
      } 
      ...
      $nm_comando = $this->Ajust_statistic($nm_comando);
      if (empty($_SESSION['sc_session'][$this->Ini->sc_page]['chart_event_doctors']['where_pesq'])) 
      { 
      ...
      { 
         $where_ok = $_SESSION['sc_session'][$this->Ini->sc_page]['chart_event_doctors']['where_pesq'] . " and " . $Where_qb ; 
      } 
      $_SESSION['scriptcase']['sc_sql_ult_comando'] = $nm_comando;
      $_SESSION['scriptcase']['sc_sql_ult_conexao'] = '';
      if (!$rt = $this->Db->Execute($nm_comando)) 
      ...
      $tot_start_date[0] = NM_encode_input(sc_strip_script($start_date)) ; 
      $tot_start_date[1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $tot_start_date[2] = (string)$rt->fields[1]; 
      $rt->Close(); 
      $$Var_name_gb = $tot_start_date;
   } 

   //-----  doctor_iddoctor
   function quebra_doctor_iddoctor_sc_free_group_by($doctor_iddoctor, $Where_qb, $Cmp_Name) 
   {
      $Var_name_gb = "SC_tot_" . $Cmp_Name;
      global $$Var_name_gb;
      $tot_doctor_iddoctor = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
          $nm_comando = "select count(*), count(distinct idevent) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['chart_event_doctors']['where_pesq']; 
      } 
      ...
      $tot_doctor_iddoctor[0] = NM_encode_input(sc_strip_script($doctor_iddoctor)) ; 
      $tot_doctor_iddoctor[1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $tot_doctor_iddoctor[2] = (string)$rt->fields[1]; 
      $rt->Close(); 
      $$Var_name_gb = $tot_doctor_iddoctor;
   } 
